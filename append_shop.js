...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('setTimeout(() => ring.remove(), 800);')) {
        cutIdx = i + 1;
        break;
    }
}
...
const cleanHead = lines.slice(0, cutIdx).join('\n');

const tailCode = `        }, delay);
    }
...
const SHOP_ITEMS = {
    clothes: [
        { id: 'clothes_default', name: '기본 의상', desc: '주인공 재의 오리지널 스타일', price: 0, image: 'jae/jae_spritesheet.png', isSprite: true, icon: '👕' },
...
function drawCustomOverlays(ctx, centerX, centerY, scale, equipped, animFrame = 0) {
    ...
    if (clothes && clothes !== 'clothes_default') {
        ...
        if (costumeImg && costumeImg.complete && costumeImg.naturalWidth > 0) {
            const costumeW = 150 * scale;
            const costumeH = 150 * scale;
    ...
    if (pet && pet !== 'pet_none') {
        ...
        if (petImg && petImg.complete && petImg.naturalWidth > 0) {
            ...
            const petW = 60 * scale;
            const petH = 60 * scale;
            const petX = centerX + 38 * scale;
            const petY = centerY - 15 * scale + petBounce;
...
window.initL1CompleteCanvas = function() {
    ...
    function renderL1Complete(timestamp) {
        ...
        if (cData && cData.idle && cData.idle.length > 0 && sSheet && sSheet.complete) {
            ...
            drawCustomOverlays(ctx, centerX, centerY, scale, getEquippedItems(), frameIdx);
        }
...
function renderShopCatalog() {
    ...
    items.forEach(item => {
        ...
        const isEquipped = shopPreviewEquipped[categoryKey] === item.id;
        ...
        const thumbHTML = item.image ? \`<img src="\${item.image}" class="item-img-thumb" alt="\${item.name}" />\` : \`<div class="item-icon">\${item.icon}</div>\`;
...
function startShopPreviewLoop() {
    ...
    function renderPreview(timestamp) {
        ...
        if (cData && cData.idle && cData.idle.length > 0 && sSheet && sSheet.complete) {
            ...
            const centerX = canvas.width / 2;
            const centerY = drawY + (f.h * scale) / 2;
            drawCustomOverlays(ctx, centerX, centerY, scale, shopPreviewEquipped, frameIdx);
        }
...
fs.writeFileSync('script.js', cleanHead + tailCode, 'utf8');
console.log('Successfully written complete script.js using Node!');
